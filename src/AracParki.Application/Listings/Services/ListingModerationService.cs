...
public sealed class ListingModerationService(IListingStore store)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!ListingStatus.Known.Contains(normalized)
            || normalized is ListingStatus.Draft or ListingStatus.Archived)
        {
    ...
    public Task ApproveAsync(string adNo, long adminAccountId, CancellationToken cancellationToken)
    {
        ...
        return store.ApproveAsync(adNo.Trim(), adminAccountId, cancellationToken);
    }
